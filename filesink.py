...
pipeline = Gst.parse_launch(
    "v4l2src ! decodebin ! vaapipostproc ! video/x-raw,format=BGRA ! videoconvert ! x264enc ! mp4mux ! filesink location=output.mp4"
)
...
try:
    ...
    # When interrupted, send an EOS event to properly finalize the file
    print("Sending EOS...")
    pipeline.send_event(Gst.Event.new_eos())

    # Wait for EOS to be handled
    bus = pipeline.get_bus()
    msg = bus.timed_pop_filtered(Gst.CLOCK_TIME_NONE, Gst.MessageType.EOS)

# Set the pipeline state to NULL to clean up resources
